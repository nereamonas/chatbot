...
df.to_csv('../archivos/csv/Videoconferenciaa_ES.csv')

#Lo editamos. borramos la priera columna y agregamos una nueva columna llamada class
df2 = pd.read_csv('../archivos/csv/Videoconferencia_ES.csv',names=["Number", "Text", "Class"])
df2.drop('number', axis = 1, inplace = True)
df2['class']='videoconferencia'
print(df2)
df2.to_csv('../archivos/csv/Videoconferencia_ES.csv', index = False)

#2- CUESTIONARIOS
#1- convertirlo en csv
df = pd.read_csv("../archivos/txt/8-Cuestionarios2_ES.txt", delimiter = '\n')
df.to_csv('../archivos/csv/Cuestionarios_ES.csv')


#en las primeras lineas hay q hacer unas pequeñas modificaciones. xq no lo coge bien. eso hay q tratarlo aparte
df2 = pd.read_csv('../archivos/csv/Cuestionarios_ES.csv',names=["Number","Text", "Class"])
df2.drop('number', axis = 1, inplace = True)
df2['class']='cuestionarios'
print(df2)
df2.to_csv('../archivos/csv/Cuestionarios_ES.csv', index = False)

# 3- CONCATENAR LOS DOS
#unir todos en un solo csv para meter al clasificador: Sin poner la primera linea
df1 = pd.read_csv('../archivos/csv/Videoconferencia_ES.csv')
df2 = pd.read_csv('../archivos/csv/Cuestionarios_ES.csv')

out = df1.append(df2)
out.to_csv('../archivos/csv/concatenate.csv', index = False)
...
"""
df3 = pd.read_csv('../archivos/csv/concatenate.csv')
clase=df3["Text"]
count=0
for x in clase:
    clase[count]=capar(x)
...
"""
df4 = pd.read_csv('../archivos/csv/VideoconferenciaXTemas_ES.csv')
clase=df4["Text"]
count=0
for x in clase:
    clase[count]=capar(x)
...
"""
df5 = pd.read_csv('../archivos/csv/CuestionariosXTemas_ES.csv')
clase=df5["Text"]
count=0
for x in clase:
    clase[count]=capar(x)
